...
export default function Page() {
  const { worlds, activeWorldId, createWorld, upsertNode, upsertEdge, removeEdge, removeNode } = useWorldStore();
  ...
  }, [selection, removeEdge, removeNode]);
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-slate-50">
        <div className="rounded-2xl border bg-white p-8 text-center shadow-soft">
          ...
          <p className="mt-2 text-slate-500">A calm canvas for worldbuilding.</p>
          <button className="mt-6 rounded-xl bg-accent px-4 py-2 text-white" onClick={() => setShowCreate(true)}>
            Create world
          </button>
        </div>
        {showCreate && (
          <div className="fixed inset-0 grid place-items-center bg-black/20">
            <div className="w-96 rounded-xl border bg-white p-5">
              <h3 className="mb-3 font-medium">Create world</h3>
              <input className="w-full rounded border p-2" value={name} onChange={(e) => setName(e.target.value)} placeholder="Beyonders" />
    ...
    <main className="h-screen">
      ...
      <div className="flex h-[calc(100vh-56px)]">
        <WorldCanvas
          ...
          onNodeCreated={(nodeId) => setSelection({ nodeId })}
        />
        <InspectorPanel
          onNodeChange={upsertNode}
          onEdgeChange={upsertEdge}
          node={world?.nodes.find((n) => n.id === selection.nodeId)}
          edge={world?.edges.find((e) => e.id === selection.edgeId)}
